...
compatibility: Requires git and a GitHub MCP server. Optionally uses Azure DevOps MCP for work item URL resolution.
metadata:
...
`[<userStoryType>#<userStory>] <Short, imperative summary of the change>`
Where `<userStoryType>` is one of `Feature`, `Bug`, or `Refactor` based on the branch type.
Examples:
`[Feature#73621] Implement user permission check in invoice flow`
`[Bug#81230] Fix null reference crash when loading settings`
`[Refactor#55110] Pricing logic for consistency`
...
2. Link the work item (e.g., `AB#<userStory>`, use resolved Azure DevOps work item URL to make a Markdown link) in the PR description within work item section. Follow additional fields required by the repo template (if detected)
